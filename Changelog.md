...
# Hellenic Diplomacy (Con-Pope)
- Added override adjacency to gametype config json
- Uploaded modified config json

## Other Changes
- Added "extension_management" to default loaded cogs in `config_defaults.toml`
- Changed `substitute_log_channels` config value to single integer from a list in `config_defaults.toml`
- Removed "superuser_only" permissions from party command `.shutdown`
