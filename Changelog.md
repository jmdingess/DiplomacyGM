...
Changed to `.edit`
- Added `bulk`, it allows you to execute `set_core`, `set_province_owner`, `set_province_half_core`, `set_total_owner` and `delete_unit` a lot of times reducing the amount of copy pasting to edit a big map.
- Added `bulk_create_units`, it allows you to create a lot of units from the same country and type.
